...
// あるNPCが既に誰かと婚姻関係にあるか
function hasSpouse(npc: NPC): boolean {
  return Object.values(npc.relationships).some(
    (rel) => MARRIAGE_WORDS.some((w) => rel.label.includes(w))
  );
}

// 恋人同士が好感度一定以上で夫婦にランクアップ
export function checkMarriage(npcs: NPC[], monogamy = false): { updated: NPC[]; marriages: Array<{ a: string; b: string }> } {
  const marriages: Array<{ a: string; b: string }> = [];
  let updated = [...npcs];
  // 今回の処理で結婚したNPC ID（一夫一婦制時に2件目をブロック）
  const newlyMarried = new Set<string>();
  ...
  for (let i = 0; i < updated.length; i++) {
    for (let j = i + 1; j < updated.length; j++) {
      ...
      // 一夫一婦制: 既に伴侶がいるNPCは新たな結婚をブロック
      if (monogamy) {
        if (hasSpouse(a) || hasSpouse(b)) continue;
        if (newlyMarried.has(a.id) || newlyMarried.has(b.id)) continue;
      }
      ...
      newlyMarried.add(a.id);
      newlyMarried.add(b.id);
    }
  }
